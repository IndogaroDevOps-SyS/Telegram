...
JNIEXPORT jint Java_id_indogaro_messenger_MediaController_startRecord(JNIEnv *env, jclass class, jstring path, jint sampleRate) {
...
JNIEXPORT jint Java_id_indogaro_messenger_MediaController_writeFrame(JNIEnv *env, jclass class, jobject frame, jint len) {
    jbyte *frameBytes = (*env)->GetDirectBufferAddress(env, frame);
    return writeFrame((uint8_t *) frameBytes, (uint32_t) len, len / 2 < frame_size);
}

JNIEXPORT void Java_id_indogaro_messenger_MediaController_stopRecord(JNIEnv *env, jclass class) {
    cleanupRecorder();
}

JNIEXPORT jint Java_id_indogaro_messenger_MediaController_isOpusFile(JNIEnv *env, jclass class, jstring path) {
    const char *pathStr = (*env)->GetStringUTFChars(env, path, 0);
...
JNIEXPORT jbyteArray Java_id_indogaro_messenger_MediaController_getWaveform2(JNIEnv *env, jclass class, jshortArray array, jint length) {
...
JNIEXPORT jbyteArray Java_id_indogaro_messenger_MediaController_getWaveform(JNIEnv *env, jclass class, jstring path) {
    const char *pathStr = (*env)->GetStringUTFChars(env, path, 0);
    if (pathStr == NULL) {
        return NULL;
    }
...
JNIEXPORT void JNICALL Java_id_indogaro_ui_Stories_recorder_FfmpegAudioWaveformLoader_init(JNIEnv *env, jobject obj, jstring pathJStr, jint count) {
...
JNIEXPORT jboolean Java_id_indogaro_messenger_MediaController_cropOpusFile(JNIEnv *env, jclass class, jstring src, jstring dst, jlong startMs, jlong endMs) {
...
JNIEXPORT jboolean Java_id_indogaro_messenger_MediaController_joinOpusFiles(JNIEnv* env, jclass class, jstring file1, jstring file2, jstring dest) {
