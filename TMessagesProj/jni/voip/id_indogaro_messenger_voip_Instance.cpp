...
public:
    ...
    void call(JNIEnv *env, jobjectArray audioSsrcs) {
        ...
        jclass clz = env->FindClass("id/indogaro/messenger/voip/VoIPService$RequestedParticipant");
...
void initWebRTC(JNIEnv *env) {
    ...
    NativeInstanceClass = static_cast<jclass>(env->NewGlobalRef(env->FindClass("id/indogaro/messenger/voip/NativeInstance")));
    DEBUG_REF("TrafficStatsClass");
    TrafficStatsClass = static_cast<jclass>(env->NewGlobalRef(env->FindClass("id/indogaro/messenger/voip/Instance$TrafficStats")));
    DEBUG_REF("FingerprintClass");
    FingerprintClass = static_cast<jclass>(env->NewGlobalRef(env->FindClass("id/indogaro/messenger/voip/Instance$Fingerprint")));
    DEBUG_REF("FinalStateClass");
    FinalStateClass = static_cast<jclass>(env->NewGlobalRef(env->FindClass("id/indogaro/messenger/voip/Instance$FinalState")));
    FinalStateInitMethod = env->GetMethodID(FinalStateClass, "<init>", "([BLjava/lang/String;Lid/indogaro/messenger/voip/Instance$TrafficStats;Z)V");
}
...
JNIEXPORT void JNICALL Java_id_indogaro_messenger_voip_NativeInstance_stopNative(JNIEnv *env, jobject obj) {
    ...
    instance->nativeInstance->stop([instance](const FinalState& finalState) {
        ...
        env->CallVoidMethod(globalRef, env->GetMethodID(NativeInstanceClass, "onStop", "(Lid/indogaro/messenger/voip/Instance$FinalState;)V"), asJavaFinalState(env, finalState));
        delete instance;
    });
}
