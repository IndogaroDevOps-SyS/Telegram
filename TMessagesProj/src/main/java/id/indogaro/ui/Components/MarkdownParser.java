...
public class MarkdownParser {
    ...
    private static int flagFor(String name) {
        ...
        switch (name.toLowerCase()) {
            case "u": case "ins":            return id.indogaro.ui.ArticleViewer.TEXT_FLAG_UNDERLINE;
            case "mark":                     return id.indogaro.ui.ArticleViewer.TEXT_FLAG_MARKED;
            case "sub":                      return id.indogaro.ui.ArticleViewer.TEXT_FLAG_SUB;
            case "sup":                      return id.indogaro.ui.ArticleViewer.TEXT_FLAG_SUP;
            case "del": case "s": case "strike": return id.indogaro.ui.ArticleViewer.TEXT_FLAG_STRIKE;
            case "b": case "strong":         return id.indogaro.ui.ArticleViewer.TEXT_FLAG_MEDIUM;
            case "i": case "em":             return id.indogaro.ui.ArticleViewer.TEXT_FLAG_ITALIC;
            case "code": case "tt":          return id.indogaro.ui.ArticleViewer.TEXT_FLAG_MONO;
            default:                         return 0;
        }
    }
    ...
    private static TL_iv.RichText materializeStyles(TL_iv.RichText rt) {
        ...
        if (rt instanceof TextStyle) {
            ...
            if ((f & id.indogaro.ui.ArticleViewer.TEXT_FLAG_MONO)      != 0) cur = wrapStyle(new TL_iv.textFixed(),       cur);
            if ((f & id.indogaro.ui.ArticleViewer.TEXT_FLAG_STRIKE)    != 0) cur = wrapStyle(new TL_iv.textStrike(),      cur);
            if ((f & id.indogaro.ui.ArticleViewer.TEXT_FLAG_UNDERLINE) != 0) cur = wrapStyle(new TL_iv.textUnderline(),   cur);
            if ((f & id.indogaro.ui.ArticleViewer.TEXT_FLAG_MARKED)    != 0) cur = wrapStyle(new TL_iv.textMarked(),      cur);
            if ((f & id.indogaro.ui.ArticleViewer.TEXT_FLAG_SUB)       != 0) cur = wrapStyle(new TL_iv.textSubscript(),   cur);
            if ((f & id.indogaro.ui.ArticleViewer.TEXT_FLAG_SUP)       != 0) cur = wrapStyle(new TL_iv.textSuperscript(), cur);
            if ((f & id.indogaro.ui.ArticleViewer.TEXT_FLAG_ITALIC)    != 0) cur = wrapStyle(new TL_iv.textItalic(),      cur);
            if ((f & id.indogaro.ui.ArticleViewer.TEXT_FLAG_MEDIUM)    != 0) cur = wrapStyle(new TL_iv.textBold(),        cur);
            return cur;
        }
