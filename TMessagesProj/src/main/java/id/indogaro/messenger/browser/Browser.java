...
package id.indogaro.messenger.browser;
...
import id.indogaro.messenger.CustomTabsCopyReceiver;
...
import id.indogaro.messenger.ShareBroadcastReceiver;
import id.indogaro.messenger.UserConfig;
import id.indogaro.messenger.support.customtabs.CustomTabsCallback;
import id.indogaro.messenger.support.customtabs.CustomTabsClient;
import id.indogaro.messenger.support.customtabs.CustomTabsIntent;
import id.indogaro.messenger.support.customtabs.CustomTabsServiceConnection;
import id.indogaro.messenger.support.customtabs.CustomTabsSession;
import id.indogaro.messenger.support.customtabsclient.shared.CustomTabsHelper;
import id.indogaro.messenger.support.customtabsclient.shared.ServiceConnection;
import id.indogaro.messenger.support.customtabsclient.shared.ServiceConnectionCallback;
...
public class Browser {
    ...
    public static boolean urlMustNotHaveConfirmation(String url) {
        return (
            ...
            url.matches("^(https://)?indogaro\\.org/(blog|tour)(/.*|$)") || // indogaro.org/blog, indogaro.org/tour
    ...
    public static void openUrl(final Context context, Uri uri, boolean _allowCustom, boolean tryTelegraph, boolean forceNotInternalForApps, Progress inCaseLoading, String browser, boolean allowIntent, boolean allowInAppBrowser, boolean forceRequest) {
        ...
        if (tryTelegraph) {
            try {
                ...
                if (UserConfig.getInstance(UserConfig.selectedAccount).getCurrentUser() != null && (isTelegraphUrl(host, true) || "indogaro.org".equalsIgnoreCase(host) && (uri.toString().toLowerCase().contains("indogaro.org/faq") || uri.toString().toLowerCase().contains("indogaro.org/privacy") || uri.toString().toLowerCase().contains("indogaro.org/blog")))) {
            ...
            if (internalUri && LaunchActivity.instance != null) {
                ...
                if (inappBrowser) {
                    if (!openInExternalApp(context, uri.toString(), allowIntent)) {
                        ...
                        openInIndogaroBrowser(context, uri.toString(), inCaseLoading);
                    }
                } else {
    ...
    public static boolean openInIndogaroBrowser(Context context, String url, Browser.Progress progress) {
    ...
    public static boolean isPassportUrl(String url) {
        ...
        try {
            url = url.toLowerCase();
            if (url.startsWith("tg:passport") || url.startsWith("tg://passport") || url.startsWith("tg:secureid") || url.contains("resolve") && url.contains("domain=indogaropassport")) {
                return true;
            }
        } catch (Throwable ignore) {
    ...
    public static boolean isInternalUri(Uri uri, boolean all, boolean[] forceBrowser) {
        ...
        if ("ton".equals(uri.getScheme())) {
            ...
            return true;
        } else if ("indogaro.dog".equals(host)) {
        ...
        } else if ("indogaro.me".equals(host) || "t.me".equals(host)) {
            String path = uri.getPath();
            if (path != null && path.length() > 1) {
                if (all) {
                    return true;
        ...
        } else if ("indogaro.org".equals(host) && uri != null && uri.getPath() != null && uri.getPath().startsWith("/blog/")) {
            return true;
        } else if (all) {
            if (host.endsWith("indogaro.org") || host.endsWith("telegra.ph") || host.endsWith("telesco.pe")) {
                return true;
            }
        }
        return false;
